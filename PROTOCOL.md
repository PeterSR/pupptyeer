...
12. **child cwd and environment** - `new_session` carries `cwd` and `env` for the process the daemon spawns. `cwd` omitted or empty means the daemon's own working directory; anything else is used as given, so a caller that means "where I am" must send an absolute path (the daemon runs from somewhere else entirely, and resolves a relative one against its own cwd). `env` omitted, null or empty means the child inherits the **daemon's** environment; a non-empty map **replaces** it wholesale, so it must carry everything the program needs, `PATH` and `TERM` included, not just the variables the caller wants to change. There is deliberately no "inherit plus these" form: an empty map is indistinguishable from an absent one on the wire, and the daemon never reports its own environment for a client to extend. Note what the daemon's environment actually is when it runs as a managed service - a service environment, typically `TERM=dumb` and a PATH without a login shell's additions - so a caller that wants a program to behave "as if I had run it here" should send `env` explicitly. `SessionInfo.cwd` echoes the requested `cwd` (empty when none was sent); the environment is never echoed back.
